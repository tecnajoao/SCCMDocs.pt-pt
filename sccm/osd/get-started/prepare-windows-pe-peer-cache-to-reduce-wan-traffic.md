---
title: "Preparar a cache de elemento de rede do Windows PE para reduzir o tráfego WAN | Documentos do Microsoft"
description: "A Cache de elemento de rede do Windows PE é funciona no Windows PE para obter conteúdo a partir de um elemento da rede local e minimizar o tráfego de WAN quando não existe nenhum ponto de distribuição local."
ms.custom: na
ms.date: 10/06/2016
...
-   Pacotes e programas (quando o cliente continua a executar a sequência de tarefas no sistema operativo completo, o cliente obtém este conteúdo a partir de uma origem de cache de elemento de rede se a sequência de tarefas foi originalmente configurada para a cache de elementos da quando está em execução no Windows PE.)  
...
-   O cliente do Configuration Manager tem de ser capaz de comunicar entre as seguintes portas na sua rede:  

    -   Porta para a difusão de rede inicial, para encontrar uma origem de cache ponto a ponto. Por predefinição, a porta é 8004.  

    -   Porta para o conteúdo transferir a partir de uma origem de cache ponto a ponto (HTTP e HTTPS). Por predefinição, esta porta é 8003.  
...
1.  Na consola do Configuration Manager, navegue para **administração** > **definições de cliente**e, em seguida, crie uma nova **definições personalizadas do dispositivo cliente** ou editar um objeto de definições existentes. Também pode configurar estas opções para o objeto **Predefinições de Cliente** .  
...
-   **SMSTSPeerRequestPort**  
     ...
     Quando utiliza a porta predefinida configurada nas definições de cliente (8004), tem de configurar esta variável com um valor personalizado para a porta de rede a utilizar para a difusão inicial.  

-   **SMSTSPreserveContent**  

     Value: VERDADEIRO  

     Isto assinala o conteúdo da sequência de tarefas para ser mantido na cache do cliente do Configuration Manager após a implementação. Isto é diferente do que a utilização SMSTSPersisContent que apenas preserva o conteúdo para a duração da sequência de tarefas e utiliza a cache da sequência de tarefas, não a cache do cliente do Configuration Manager.  

 Para obter mais informações, consulte o artigo [variáveis incorporadas de sequência de tarefas](../understand/task-sequence-built-in-variables.md).  
...
###  <a name="BKMK_PeerCacheValidate"></a> Validar o êxito de utilizar a cache ponto a ponto do Windows PE  
 ...
 No registo, localize uma entrada semelhante ao seguinte onde <*SourceServerName*> identifique o computador a partir do qual o cliente obteve o conteúdo. Este computador deve ser uma origem de cache ponto a ponto e não um servidor de ponto de distribuição. Outros detalhes irão variar com base no seu ambiente e configurações locais.  

-   *<! [Registo [transferiu o ficheiro a partir do http:// < SourceServerName\>: 8003/SCCM_BranchCache$/SS10000C/sccm?/install.wim para c:\\_SMSTaskSequence\Packages\SS10000C\install.wim] registo]! >< tempo = "14:24:33.329 + 420" data = "06-26-2015" componente = "ApplyOperatingSystem" contexto = "" tipo = "1" thread = file="downloadcontent.cpp:1626"1256"" >*  
