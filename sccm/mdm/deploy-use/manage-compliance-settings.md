...
Esses cenários oferecem uma introdução à utilização de definições de compatibilidade do System Center Configuration Manager ao trabalhar com alguns cenários comuns que poderá encontrar.  

 Se já estiver familiarizado com as definições de conformidade, pode encontrar documentação detalhada sobre todas as funcionalidades que utiliza no [itens de configuração para dispositivos geridos com o Intune](#configuration-items-for-devices-managed-with-intune) secção.  

 [Introdução às definições de conformidade](../../compliance/get-started/get-started-with-compliance-settings.md) fornece as noções básicas sobre as definições de compatibilidade e [planear e configurar as definições de compatibilidade](../../compliance/plan-design/plan-for-and-configure-compliance-settings.md) irá ajudá-lo a implementar os pré-requisitos necessários.  
...
4.  No separador **Geral** do Assistente de Criação de Item de Configuração, conforme apresentado abaixo, especifique um nome e uma descrição para o item de configuração e escolha o tipo de item de configuração adequado para cada cenário deste tópico.  
     ...
     ![Mostra uma página geral do Assistente de item de configuração de criar.](media/Compliance-Settings-Wizard---1.png)  
...
2. Sobre o **plataformas suportadas** , selecione todas as plataformas Windows 10.  
...
2. Sobre o **plataformas suportadas** página, selecione **todos os Windows Phone 8.1** plataformas.  
...
2. Sobre o **plataformas suportadas** , selecione todos os iPhone e todas as plataformas de dispositivos iPad.  
...
2. Sobre o **plataformas suportadas** , selecione apenas **Android 5** (para garantir que as definições são aplicadas apenas nessa plataforma).  
...
Os seguintes tipos de item de configuração do System Center Configuration Manager estão disponíveis para dispositivos que não são geridos pelo cliente do Configuration Manager, por exemplo, os dispositivos que estão inscritos no Microsoft Intune.  
 ...
 -   [Como criar itens de configuração para dispositivos Android e Samsung KNOX Standard gerido com o Intune](create-configuration-items-for-android-and-samsung-knox-devices-managed-without-the-client.md)  
