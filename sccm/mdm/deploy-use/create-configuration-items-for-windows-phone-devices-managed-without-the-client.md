---
title: "Como criar itens de configuração para dispositivos Windows Phone geridos com o Intune | Documentos do Microsoft"
ms.custom: na
ms.date: 03/05/2017
...
Utilizar o System Center Configuration Manager**Windows Phone** item de configuração para gerir as definições para dispositivos Windows Phone que estão inscritos no Microsoft Intune ou no local gerido pelo Configuration Manager.  
...
5.  Em **especifique o tipo de item de configuração que pretende criar**, selecione **Windows Phone**.  
...
|Definição|Detalhes|  
|-------------|-------------|  
|**Captura de ecrã**|Permitir que o utilizador efetuar uma captura de ecrã do ecrã do dispositivo.<br /><br /> (Apenas no Windows Phone 8.1)|  
|**Submissão de dados de diagnóstico**|Permitir a submissão de ficheiros de registo de aplicações.|  
|**Geolocalização**|Permitir que o dispositivo utilize informações dos serviços de localização.<br /><br /> (Apenas no Windows Phone 8.1)|  
|**Copiar e Colar**|Utilizar a ação copiar e colar para transferir dados entre aplicações.<br /><br /> (Apenas no Windows Phone 8.1)|  
...
### <a name="store"></a>Arquivo  
 Estas definições aplicam-se a apenas para dispositivos Windows Phone 8.1.  
...
|**Conta Microsoft**|Permitir a utilização de uma conta Microsoft no dispositivo.<br /><br /> (Apenas no Windows Phone 8.1)|  
...
|**Comunicação de proximidade (NFC)**|Permitir a comunicação através de NFC no dispositivo.<br /><br /> (Apenas no Windows Phone 8.1)|  
|**Permitir ligação USB**|Permita dos periféricos, tais ligar a este dispositivo através de USB.|
...
|Definição|Detalhes|  
|-------------|-------------|  
|**Especificar a hora de ponta**|Especifique uma janela de tempo que será utilizada pelas duas definições seguintes.|  
|**Frequência de sincronização dentro do período de ponta**|Escolha a frequência o dispositivo sincronizará durante o período de ponta que especificou.|  
|**Frequência de sincronização fora do período de ponta**|Escolha a frequência o dispositivo sincronizará fora da hora de ponta que especificou.|  
...
|Definição|Detalhes|  
|-------------|-------------|  
|**Gestão de dispositivos no modo de roaming**|Permite que o dispositivo para serem geridos pelo Configuration Manager quando este estiver em roaming.|  
...
|**Permitir manual unenrollment**|Vamos o utilizador remover dispositivo da MDM.|  
...
### <a name="allowed-and-blocked-apps-list-windows-phone-81-only"></a>Lista de aplicações permitidas e bloqueadas (apenas no Windows Phone 8.1)  
 ...
 Não é possível especificar ambas permitidas em bloqueadas aplicações no mesmo item de configuração.  
  
> [!IMPORTANT]  
>  Se especificar uma lista de aplicações permitidas, tem de garantir que a aplicação do portal da empresa e todas as aplicações que implementou para dispositivos Windows Phone 8.1 estão a **permitido** lista de aplicações.  
  
##### <a name="to-specify-an-allowed-or-blocked-apps-list"></a>Para especificar uma lista de aplicações permitida ou bloqueada  
  
1.  No **permitidos e bloqueados aplicações lista (Windows Phone 8.1)** página, especifique as seguintes informações:  
