...
## <a name="capacity-planning-recommendations-for-software-updates"></a>Recomendações de planeamento da capacidade para atualizações de software  
 Pode utilizar as seguintes recomendações como uma linha base que pode ajudá-lo a determinar as informações para o planeamento da capacidade de atualizações de software que é adequado para a sua organização. Os requisitos de capacidade real podem variar das recomendações que são apresentadas neste artigo, consoante os seguintes critérios: o ambiente de rede específico, o hardware que utiliza para alojar o sistema de sites de ponto de atualização de software, o número de clientes que estão instalados e as funções de sistema de sites que estão instaladas no servidor.  
...
-   Por predefinição, o Configuration Manager não suporta configurar pontos de atualização de software como clusters de NLB. Antes do Configuration Manager versão 1702, pode utilizar o SDK do Configuration Manager para configurar até quatro pontos de atualização de software num NLB cluster. No entanto, a partir do Configuration Manager versão 1702, pontos de atualização de software não são suportados como clusters de NLB e atualizações do Configuration Manager versão 1702 serão bloqueadas se esta configuração é detetada.
...
##  <a name="BKMK_SUPInfrastructure"></a> Determinar a infraestrutura do ponto de atualização de software  
 O site de administração central e todos os sites primários subordinados têm de ter um ponto de atualização de software para implementar atualizações de software. Quando planear a infraestrutura de ponto de atualização de software, é necessário determinar as seguintes dependências:
...
2.  O cliente tentará novamente um mínimo de quatro vezes em intervalos de 30 minutos. Após a quarta falha, e depois de aguardar dois minutos adicionais, o cliente desloca-se para o ponto de atualização de software seguinte na lista de pontos de atualização de software.  
...
-   Se a gestão de clientes baseada na Internet está ativada e existirem vários pontos de atualização de software que estão configurados para aceitar comunicações de clientes na Internet, o processo de mudança segue o processo de repetição padrão que é descrito no anterior cenário.  
...
Quando o Gestor de configuração recebe qualquer um dos seguintes códigos de erro do agente do Windows Update, tem de repetição de cliente a ligação:  
...
###  <a name="BKMK_NLBSUPSP1"></a> Ponto de atualização de software configurado para utilizar um NLB  
 Provavelmente mudança de ponto de atualização de software aborda as necessidades de tolerância de falhas que tiver. Por predefinição, o Configuration Manager não suporta configurar pontos de atualização de software como clusters de NLB. Antes do Configuration Manager versão 1702, pode utilizar o SDK do Configuration Manager para configurar até quatro pontos de atualização de software num NLB cluster. No entanto, a partir do Configuration Manager versão 1702, pontos de atualização de software não são suportados como clusters de NLB e atualizações do Configuration Manager versão 1702 serão bloqueadas se esta configuração é detetada. Para obter mais informações sobre o cmdlet Set-CMSoftwareUpdatePoint do PowerShell, consulte o [Set-CMSoftwareUpdatePoint](http://go.microsoft.com/fwlink/?LinkId=276834).

###  <a name="BKMK_SUPSecSite"></a> Ponto de atualização de software num site secundário  
 O ponto de atualização de software é opcional num site secundário. Apenas pode instalar um ponto de atualização de software num site secundário. Os dispositivos atribuídos a um site secundário são configurados para utilizar um ponto de atualização de software no site principal quando não existe um ponto de atualização de software instalado no site secundário. Normalmente, instala um ponto de atualização de software num site secundário quando existe largura de banda limitada entre os dispositivos que estão atribuídos ao site secundário e os pontos de atualização de software no site primário principal, ou quando o ponto de atualização de software se aproxima o limite de capacidade. Após a instalação e configuração de um ponto de atualização de software com êxito no site secundário, é atualizada para todo o site uma política para os computadores cliente que estejam atribuídos ao site, passando estes a utilizar o novo ponto de atualização de software.  
...
####  <a name="BKMK_WSUSandSSL"></a> Decidir se pretende configurar o WSUS para utilizar SSL  
 ...
 Quando instalar e configurar o ponto de atualização de software, terá de selecionar a definição **Ativar as comunicações SSL para o Servidor WSU** . Caso contrário, o Configuration Manager configura o WSUS para não utilizar SSL. Quando ativar SSL para o WSUS que é executado num ponto de atualização de software, o WSUS que é executado no ponto de atualização de software em quaisquer sites subordinados têm também de ser configurados para a utilização de SSL.  
...
-   Se não tiver sido aprovada uma atualização de software de substituição para implementação no ambiente de produção.  
    ...
    > [!NOTE]  
    > Quando o Configuration Manager define uma atualização de software substituídas, tal como **expirado**, defini-lo não a atualização **recusada** no WSUS. No entanto, quando a tarefa de limpeza do WSUS é executada, as atualizações como **expirado** no Configuration Manager estão definidos para um Estado de **recusada** no WSUS server e o Windows Update Agent nos computadores já não irão analisar estas atualizações. Isto significa que os clientes continuam a análise de atualização expirada até que executa a tarefa de limpeza. Para obter informações sobre a tarefa de limpeza do WSUS, consulte [manutenção de atualizações de Software](/sccm/sum/deploy-use/software-updates-maintenance).

###  <a name="BKMK_UpdateLanguages"></a> Idiomas  
 As definições de idioma para o ponto de atualização de software permitem-lhe configurar os idiomas para os quais os detalhes de resumo (metadados de atualizações de software) estão sincronizados para atualizações de software e os idiomas de ficheiros de atualização de software que são transferidos para o software atualizações.  
