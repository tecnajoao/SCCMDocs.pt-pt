...
- [Dashboard de gestão de clientes do Office 365](#office-365-client-management-dashboard): Pode rever as informações de cliente do Office 365 do dashboard de gestão de clientes do Office 365. A partir do Configuration Manager versão 1802, o dashboard de gestão de cliente do Office 365 apresenta uma lista de dispositivos relevantes quando estão selecionadas secções de gráfico. <!--1357281 -->
...
3. No **definições da aplicação** página, forneça um nome e descrição para a aplicação, introduza a localização de transferência para os ficheiros e, em seguida, clique em **seguinte**. A localização tem de ser especificada como &#92; &#92; *servidor*&#92;*partilhar*.
4. No **importar as definições de cliente** página, escolha se pretende importar as definições de cliente do Office 365 a partir de um ficheiro de configuração XML existente ou especifique manualmente as definições. Clique em **seguinte** quando tiver terminado.  

    Quando tiver um ficheiro de configuração existente, introduza a localização do ficheiro e avance para o passo 7. Tem de especificar a localização no formato &#92; &#92; *servidor*&#92;*partilhar*&#92;*filename*. XML.
    > [!IMPORTANT]    
    > O ficheiro de configuração XML tem de conter apenas [idiomas suportados pelo cliente do Office 365 ProPlus](/DeployOffice/office2016/language-identifiers-and-optionstate-id-values-in-office-2016).
...
3.  Ative clientes do Office 365 receber atualizações do Configuration Manager. Definições de cliente utilize o Gestor de configuração ou política de grupo para permitir ao cliente.   
    ...
    **Método 2**: [Ativar clientes do Office 365 receber atualizações](/DeployOffice/manage-updates-to-office-365-proplus-with-system-center-configuration-manager#BKMK_EnableClient) do Configuration Manager utilizando a ferramenta de implementação do Office ou a política de grupo.  
...
|1802| O cliente recebe as notificações de pop-up e na aplicação, bem como uma caixa de diálogo de contagem decrescente, antes de instalar a atualização. </br>Se estiver a executar todas as aplicações do Office 365 durante uma imposição de atualização de cliente do Office 365, as aplicações do Office não ser forçadas a fechar. Em vez disso, a instalação da atualização devolverá como necessitando de um reinício do sistema <!--510006-->|
...
>- A notificação e a caixa de diálogo de contagem decrescente foi possível iniciar enquanto o utilizador não está a funcionar ativamente no dispositivo. Por exemplo, quando o dispositivo está bloqueado overnight é possíveis Office aplicações em execução no dispositivo pode ser forçada fechar para instalar a atualização. Antes de fechar a aplicação, o Office guarda os dados da aplicação para evitar a perda de dados. 
...
3. Clique em **consulta**, e, em seguida, execute a seguinte consulta: *selecione &#42; de SMS_SCI_Component onde componentname = "SMS_WSUS_CONFIGURATION_MANAGER"*  
...
## <a name="updating-office-365-during-task-sequences-when-office-365-is-installed-in-the-base-image"></a>A atualização do Office 365 durante a sequências de tarefas quando o Office 365 é instalado na imagem de base
Quando instala um sistema operativo em que o Office 365 já está instalada na imagem, é possível que o valor de chave do registo de canal de atualização tem a localização de instalação original. Neste caso, a análise de atualização não apresentará quaisquer atualizações de cliente do Office 365 conforme aplicável. Há uma tarefa agendada de atualizações automáticas Office que é executada várias vezes por semana. Após a execução da tarefa, o canal de atualização irá apontar para o URL de CDN configurado do Office e a análise, em seguida, irá mostrar estas atualizações, conforme aplicável. <!--510452-->

Para garantir que o canal de atualização está definido para que as atualizações aplicáveis serão consideradas, efetue os seguintes passos:
1. Num computador com a mesma versão do Office 365 como a imagem base do SO, abra o Programador de tarefas (taskschd.msc) e identificar a tarefa de atualizações automáticas do Office 365. Normalmente, este ficará localizado em **biblioteca do Programador de tarefas** >**Microsoft**>**Office**.
2. O rato sobre o automático de atualizações de tarefas e selecione **propriedades**.
...
5. Adicione um novo **executar linha de comandos** passo antes do **instalar atualizações** passo da sequência de tarefas. 
6. Copiar o comando e os argumentos que recolheu da tarefa agendada de atualizações automáticas do Office. 
7. Clique em **OK**. 
