---
title: Ficheiros de registo para o Configuration Manager | Documentos do Microsoft
...
No System Center Configuration Manager, componentes de servidor de cliente e o site registam as informações do processo em ficheiros de registo individuais. Pode utilizar as informações nos ficheiros de registo para o ajudar a resolver problemas que possam ocorrer na hierarquia do Configuration Manager. Por predefinição, o registo de componente de cliente e o servidor está ativado no Configuration Manager.   

 As secções seguintes fornecem detalhes sobre os ficheiros de registo diferentes disponíveis para si. Pode utilizar esta informação para ver e monitorizar o cliente do Configuration Manager e os registos do servidor para os detalhes da operação, e para identificar erros informações que poderão ajudar a resolver quaisquer problemas.  

-   [Sobre ficheiros de registo do Configuration Manager](#BKMK_AboutLogs)  

    -   [Configurar opções de registo utilizando o serviço do Configuration Manager](#BKMK_LogOptions)  
...
-   [Registos de cliente do Configuration Manager](#BKMK_ClientLogs)  
    ...
    -   [Ficheiros de registo de instalação de cliente](#BKMK_ClientInstallLog)  
...
-   [Ficheiros de registo de servidor de site do Configuration Manager](#BKMK_ServerLogs)  
    ...
    -   [Servidor e o site de sistema registos do servidor sites](#BKMK_SiteSiteServerLog)  
...
-   [Ficheiros de registo para funcionalidade do Configuration Manager](#BKMK_FunctionLogs)  
    ...
    -   [O Asset intelligence](#BKMK_AILog)  
    ...
    -   [Gateway de gestão da nuvem](#cloud-management-gateway)
...
##  <a name="BKMK_AboutLogs"></a>Sobre ficheiros de registo do Configuration Manager  
 A maioria dos processos no Configuration Manager escreverem informações operacionais num ficheiro de registo que é dedicado a esse processo. Os ficheiros de registo são identificados pela **. log** ou **. lo _** extensões de ficheiros. O Configuration Manager escreve num ficheiro. log até que esse registo atinja o tamanho máximo. Quando o registo está cheio, o ficheiro. log é copiado para um ficheiro com o mesmo nome mas com a extensão. lo _ e o processo ou componente continua a escrever o ficheiro. log. Quando o ficheiro. log atinge novamente o tamanho máximo, o ficheiro. lo _ é substituído e o processo é repetido. Alguns componentes criam um histórico de ficheiros de registo, acrescentando um carimbo de data e hora ao nome de ficheiro de registo e mantendo a extensão. log. Uma exceção ao tamanho máximo e a utilização do ficheiro. lo _ é o cliente para Linux e UNIX. Para obter informações sobre como o cliente para Linux e UNIX utiliza ficheiros de registo, consulte o artigo [ficheiros de registo de gerir o cliente para Linux e UNIX](#BKMK_ManageLinuxLogs) neste tópico.  

 Para ver os registos, utilize a ferramenta do Configuration Manager registo Visualizador CMTrace, localizada no \\SMSSetup\\pasta Ferramentas de suporte de dados de origem do Configuration Manager. A ferramenta CMTrace é adicionada a todas as imagens de arranque que são adicionadas à biblioteca de Software.  

###  <a name="BKMK_LogOptions"></a>Configurar opções de registo utilizando o serviço do Configuration Manager  
 No Configuration Manager, pode alterar onde os ficheiros de registo são armazenados e pode alterar o tamanho do ficheiro de registo.  

 Para modificar o tamanho dos ficheiros de registo, altere o nome e a localização do ficheiro de registo, ou para forçar vários componentes a escreverem num ficheiro de registo único, efetue os seguintes passos.  
...
2.  No **base** separador o **componente** grupo, selecione **iniciar**e, em seguida, selecione **do serviço do Configuration Manager**.  
3.  Quando o serviço do Configuration Manager abre-se, ligar para o site que pretende gerir. Se o site que pretende gerir não estiver apresentada, selecione **Site**, selecione **ligar**e, em seguida, introduza o nome do servidor do site do site correto.  
4.  Expandir o site e aceda ao **componentes** ou **servidores**, dependendo do local onde os componentes que pretende gerir estiverem localizados.  
...
|CITaskMgr.log|Regista tarefas que são iniciadas de cada aplicação e o tipo de implementação, tais como conteúdo transferirem e instalar ou desinstalar ações.|  
|ClientAuth.log|Regista a assinatura e a actividade de autenticação de cliente.|  
...
|CMHttpsReadiness.log|Regista os resultados da execução a ferramenta de avaliação de preparação do Configuration Manager HTTPS. Esta ferramenta verifica se os computadores têm um certificado de autenticação de cliente de infraestrutura de chaves públicas (PKI) que pode ser utilizado com o Configuration Manager.|  
|CmRcService.log|Regista informações para o serviço de controlo remoto.|  
|ContentTransferManager.log|Agenda o serviço de transferência inteligente em segundo plano em segundo plano (BITS) ou Server Message Block (SMB) para transferir ou aceder aos pacotes.|  
...
|MaintenanceCoordinator.log|Regista a atividade de tarefas de manutenção geral para o cliente.|  
...
|Ficheiro sleepagent _&lt;*domínio*\>@SYSTEM_0.log|O ficheiro de registo principal do proxy de reativação.|  
...
|SWMTRReportGen.log|Gera um relatório de dados de utilização que é recolhido pelo agente de medição. Estes dados são registados no ficheiro Mtrmgr.log.|  
...
|wakeprxy-uninstall.log|Regista informações sobre a desinstalação do proxy de reativação quando os clientes recebem o definição de cliente opção para desativar o proxy de reativação, caso o proxy de reativação tiver sido ativado.|  

###  <a name="BKMK_ClientInstallLog"></a>Ficheiros de registo de instalação de cliente  
...
|Nome do registo|Descrição|  
|--------------|-----------------|  
|ccmsetup.log|Regista tarefas do ccmsetup.exe para configuração de clientes, a atualização de cliente e remoção de cliente. Pode ser utilizado para resolver problemas de instalação de cliente.|  
...
> [!TIP]  
>  A partir com clientes Linux e UNIX de 1 de atualização cumulativa, pode utilizar CMTrace para ver os ficheiros de registo de cliente para Linux e UNIX.  
...
|Scxcmprovider.log|O ficheiro de registo do serviço CIM do cliente do Configuration Manager para Linux e UNIX (omiserver). Este ficheiro de registo contém informações sobre as operações do nwserver.bin em curso.<br /><br /> Este registo está localizado em**/var/opt/microsoft/configmgr/scxcmprovider.log**<br /><br /> Para alterar a localização do ficheiro de registo, edite **/opt/microsoft/omi/etc/scxcmprovider.conf** e altere o campo **PATH**. Não é necessário reiniciar o serviço ou o computador cliente para que a alteração produza efeito.<br /><br /> Pode definir o nível de registo para uma das três definições.|  
...
-   **scxcm.log**. Para alterar o nível de registo, edite **/opt/microsoft/configmgr/etc/scxcm.conf** e altere cada instância do **módulo** etiqueta para o nível de registo que pretende:  
    ...
    -   INFORMAÇÕES: Registo mais detalhado que indica o estado de vários eventos no cliente  
...
Em condições normais, utilize o nível de registo de erro. Este nível de registo cria o ficheiro de registo mais pequeno. À medida que o nível de registo aumenta de ERROR para aviso, informações e, em seguida, para TRACE, é criado um ficheiro de registo maior que são escritos mais dados para o ficheiro.  

####  <a name="BKMK_ManageLinuxLogs"></a>Gerir ficheiros de registo para o cliente de Linux e UNIX  
O cliente para Linux e UNIX não limita o tamanho máximo dos ficheiros de registo de cliente, nem o cliente copia automaticamente o conteúdo dos ficheiros. log para outro ficheiro, tal como para um ficheiro. lo _. Se pretender controlar o tamanho máximo dos ficheiros de registo, implemente um processo para gerir os ficheiros de registo independentes do cliente do Configuration Manager para Linux e UNIX.  

Por exemplo, pode utilizar o comando padrão Linux e UNIX **logrotate** para gerir o tamanho e rotação dos ficheiros de registo de cliente. O cliente do Configuration Manager para Linux e UNIX tem uma interface que permite que **logrotate** para assinalar o cliente quando tiver concluído a rotação de registo, por isso, o cliente pode retomar o registo para o ficheiro de registo.  
...
|CCMAgent -&lt;*date_time*>. log|Regista informações relacionadas com operações de cliente, incluindo o início de sessão do utilizador e operações de terminar sessão e da atividade do computador Mac.<br /><br /> Este ficheiro de registo encontra-se na pasta do ~/Library/Logs no computador Mac.|  
|CCMNotifications -&lt;*date_time*>. log|Regista atividades relacionadas com o Configuration Manager notificações apresentadas no computador Mac.<br /><br /> Este ficheiro de registo está localizado na pasta ~/Library/Logs no computador Mac.|  
...
M o ficheiro de registo sms_dm no servidor do sistema de site também regista a comunicação entre computadores Mac e o ponto de gestão que está configurado para dispositivos móveis e computadores Mac.  

##  <a name="BKMK_ServerLogs"></a>Ficheiros de registo de servidor de site do Configuration Manager  
 As secções seguintes listam os ficheiros de registo que estejam no servidor do site ou que estão relacionadas com funções de sistema de sites específicas.  

###  <a name="BKMK_SiteSiteServerLog"></a>Servidor e o site de sistema registos do servidor sites  
 A tabela seguinte lista os ficheiros de registo que se encontram no servidor do site do Configuration Manager e servidores do sistema de sites.  
...
|CertMgr.log|Registos de certificado atividades para comunicações intra-site.|Servidor do sistema de sites|  
...
|dataldr.log|Regista informações sobre o processamento de ficheiros MIF e de inventário de hardware na base de dados do Configuration Manager.|Servidor do site|  
...
|EPCtrlMgr.log|Regista informações sobre a sincronização de informações de ameaças de software maligno a partir do servidor de funções de sistema de sites do Endpoint Protection com a base de dados do Configuration Manager.|Servidor do site|  
...
|hman.log|Regista informações sobre alterações da configuração do site e sobre a publicação de informações do site nos serviços de domínio do Active Directory.|Servidor do site|  
...
|migmctrl.log|Regista informações de ações de migração que envolvam tarefas de migração, pontos de distribuição partilhados e atualizações de ponto de distribuição.|Site de nível superior na hierarquia do Configuration Manager e cada site primário subordinado.<br /><br /> Numa hierarquia com múltiplos sites primários, utilize o ficheiro de registo criado no site de administração central.|  
...
|ResourceExplorer.log|Regista erros, avisos e informações sobre a execução do Explorador de recursos.|Computador que executa a consola do Configuration Manager|  
...
|Nome do registo|Descrição|Computador com o ficheiro de registo|  
|--------------|-----------------|----------------------------|  
|ConfigMgrPrereq.log|Regista componentes de pré-requisitos instalação e avaliação atividades.|Servidor do site|  
...
|MP_Ddr.log|Regista a conversão de XML registos a partir de clientes e, em seguida, copia-os para o servidor do site.|Servidor do sistema de sites|  
...
|MP_OOBMgr.log|Regista as atividades do ponto de gestão relacionados com a receção de um OTP de um cliente.|Servidor do sistema de sites|  
...
|Nome do registo|Descrição|Computador com o ficheiro de registo|  
|--------------|-----------------|----------------------------|  
|objreplmgr.log|Regista detalhes sobre a replicação do software de atualizações a ficheiros de notificação de um site principal para sites subordinados.|Servidor do site|  
...
|WCM.log|Regista detalhes sobre a atualização de software ponto a configuração e de ligações ao servidor WSUS para idiomas, classificações e categorias de atualização subscritas.|Servidor do site que liga ao servidor WSUS|  
...
|WUSSyncXML.log|Regista detalhes sobre a ferramenta de inventário para o Microsoft Updates sincronizar processo.|Computador cliente configurado como o anfitrião de sincronização da ferramenta de inventário para o Microsoft Updates|  

##  <a name="BKMK_FunctionLogs"></a>Ficheiros de registo para funcionalidade do Configuration Manager  
 A lista de secções seguinte ficheiros relacionados com funções do Gestor de configuração de registo.  
...
|awebsctl.log|Regista a monitorização de atividades para a função de sistema de sites de ponto do catálogo de aplicações web service.|Servidor do sistema de sites|  
...
###  <a name="BKMK_AILog"></a>O Asset intelligence  
...
|AIUSMSI.log|Regista detalhes sobre a instalação da sincronização do Asset Intelligence função de sistema de sites do ponto.|Servidor do sistema de sites|  
|AIUSSetup.log|Regista detalhes sobre a instalação da sincronização do Asset Intelligence função de sistema de sites do ponto.|Servidor do sistema de sites|  
...
|Nome do registo|Descrição|Computador com o ficheiro de registo|  
|--------------|-----------------|----------------------------|  
|ConfigMgrSetup.log|Regista informações sobre tarefas de configuração e recuperação quando o Configuration Manager recupera um site a partir da cópia de segurança.|Servidor do site|  
|Smsbkup.log|Regista os detalhes sobre a atividade de cópia de segurança do site.|Servidor do site|  
|smssqlbkup.log|Regista a saída do processo de cópia de segurança de base de dados do site quando o SQL Server estiver instalado num servidor que não seja o servidor de site.|Servidor da base de dados do site|  
...
###  <a name="BKMK_CertificateEnrollment"></a>Inscrição de certificados  
 A tabela seguinte lista os ficheiros de registo do Configuration Manager que contêm informações relacionadas com a inscrição de certificados. Inscrição de certificados utiliza o ponto de registo de certificados e o módulo de política do Configuration Manager no servidor que está a executar o serviço de inscrição de dispositivos de rede.  
...
|NDESPlugin.log|Registos desafio de verificação e as atividades de inscrição de certificado.|Módulo de política do Configuration Manager e o serviço de inscrição de dispositivos de rede|  

 Além dos ficheiros de registo do Configuration Manager, analise os registos de aplicação do Windows no Visualizador de eventos do servidor que executa o serviço de inscrição de dispositivos de rede e o servidor que aloja o ponto de registo de certificados. Por exemplo, procure mensagens da origem **NetworkDeviceEnrollmentService**. Também pode utilizar os seguintes ficheiros de registo:  
...
|Nome do registo|Descrição|Computador com o ficheiro de registo|  
|--------------|-----------------|----------------------------|  
|bgbmgr.log|Regista detalhes sobre atividades de servidor de site relacionadas com tarefas de notificação do cliente e o processamento online e a ficheiros de estado da tarefa.|Servidor do site|  
|BGBServer.log|Regista as atividades do servidor de notificação, como comunicações cliente-servidor e envio de tarefas para os clientes. Regista também informações sobre a geração do online e os ficheiros de estado de tarefa para serem enviados para o servidor do site.|Ponto de gestão|  
|BgbSetup.log|Regista as atividades do processo do wrapper de instalação de servidor notificação durante a instalação e desinstalação.|Ponto de gestão|  
...
|CcmNotificationAgent.log|Regista as atividades do agente de notificação, como comunicações cliente-servidor e informações sobre tarefas de recebidas e distribuídas por outros agentes de cliente.|Cliente|  
...
|-|-|-|
|Nome do registo|Descrição|Computador com o ficheiro de registo|
|CloudMgr.log|Regista detalhes sobre como implementar o serviço de gateway de gestão de nuvem, estado do serviço em curso e dados de utilização associados ao serviço.<br>Pode configurar o nível de registo ser a edição do registo **HKEY_LOCAL_MACHINE\SOFTWARE\Microsoft\SMS\COMPONENTS\SMS_CLOUD_SERVICES_MANAGER\Logging nível**|O **SMS/Logs** pasta no servidor do sistema de sites|
|CMGSetup.log ou CMG -*RoleInstanceID*-CMGSetup.log<sup>1</sup>|Regista detalhes sobre a 2ª fase da implementação de gateway de gestão da nuvem (implementação local no Azure)<br>Pode configurar o nível de registo utilizando a definição **nível de rastreio** (**informações** (predefinição), **verboso**, **erro**) no **configuração de serviços do Azure portal\Cloud** separador.|O **%approot%\logs** no seu servidor do Azure ou a pasta SMS/Logs no servidor do sistema de sites|
|CMGHttpHandler.log ou CMG -*RoleInstanceID*-CMGHttpHandler.log<sup>1</sup>|Regista detalhes sobre o enlace de processador de http de gateway de gestão do nuvem com o serviços de informação de Internet no Azure<br>Pode configurar o nível de registo utilizando a definição **nível de rastreio** (**informações** (predefinição), **verboso**, **erro**) no **configuração de serviços do Azure portal\Cloud** separador.|O **%approot%\logs** no seu servidor do Azure ou a pasta SMS/Logs no servidor do sistema de sites|
|CMGService.log ou CMG -*RoleInstanceID*-CMGService.log<sup>1</sup>|Regista detalhes sobre o componente de principais de serviço de gateway de gestão do nuvem no Azure<br>Pode configurar o nível de registo utilizando a definição **nível de rastreio** (**informações** (predefinição), **verboso**, **erro**) no **configuração de serviços do Azure portal\Cloud** separador.|O **%approot%\logs** no seu servidor do Azure ou a pasta SMS/Logs no servidor do sistema de sites|
...
<sup>1</sup> tratam local ficheiros de registo do Configuration Manager na nuvem a sincronização de Gestor do serviço de armazenamento do Azure cada 5 minutos. O gateway de gestão da nuvem transmite os registos de armazenamento do Azure cada 5 minutos. Assim, o atraso máximo será 10 minutos. Comutadores verbosos afetará registos locais e remotos.

- Para resolver problemas com implementações, utilize **Cloudmgr** e **CMGSetup.log**
...
|DcmWmiProvider.log|Regista informações sobre a leitura de synclets de itens a configuração do WMI.|Cliente|  
...
|Nome do registo|Descrição|Computador com o ficheiro de registo|  
|--------------|-----------------|----------------------------|  
|Ficheiro CloudDP -&lt;guid\>. log|Regista os detalhes de um ponto de distribuição baseado na nuvem específico, incluindo informações sobre armazenamento e acesso ao conteúdo.|Servidor do sistema de sites|  
|CloudMgr.log|Regista detalhes sobre o aprovisionamento de conteúdo, recolha ações iniciadas pelo administrador para interromper ou iniciar o serviço em nuvem que executa um ponto de distribuição baseado na nuvem, armazenamento e as estatísticas de largura de banda.|Servidor do sistema de sites|  
|DataTransferService.log|Regista todas as comunicações BITS para acesso a políticas ou pacotes. Este registo também é utilizado para gestão de conteúdos por pontos de distribuição de solicitação.|Computador que esteja configurado como um ponto de distribuição de solicitação|  
|PullDP.log|Regista os detalhes sobre o conteúdo que o ponto de distribuição de extração transfere dos pontos de distribuição de origem.|Computador que esteja configurado como um ponto de distribuição de solicitação|  
|PrestageContent.log|Regista os detalhes sobre a utilização da ExtractContent.exe ferramenta num ponto de distribuição remoto, pré-configurado. Esta ferramenta extrai o conteúdo exportado para um ficheiro.|Função do sistema de sites|  
|SMSdpmon.log|Regista detalhes sobre o estado de funcionamento do ponto de distribuição monitorização tarefas agendadas que são configuradas no ponto de distribuição.|Função do sistema de sites|  
|smsdpprov.log|Regista os detalhes sobre a extração de ficheiros comprimidos recebidos de um site primário. Este registo é gerado pelo fornecedor WMI do ponto de distribuição remoto.|Computador de ponto de distribuição que não está colocalizada com o servidor do site|  
...
|EPCtrlMgr.log|Regista detalhes sobre a sincronização de informações de ameaças de software maligno do servidor da função Endpoint Protection com a base de dados do Configuration Manager.|Servidor do sistema de sites|  
...
|SMS_DM.log|Regista a comunicação entre dispositivos móveis e computadores Mac e o ponto de gestão ativado para dispositivos móveis e computadores Mac.|Servidor do sistema de sites|  
...
|ccmsetup.log|Regista tarefas do ccmsetup para a configuração de cliente, a atualização de cliente e remoção de cliente. Pode ser utilizado para resolver problemas de instalação de cliente.|Cliente|  
...
|Dism.log|Regista ações de instalação de controladores ou ações de aplicação de atualização para manutenção offline.|Servidor do sistema de sites|  
|Distmgr.log|Regista detalhes sobre a configuração da ativação de um ponto de distribuição para o ambiente de execução de Preboot (PXE).|Servidor do sistema de sites|  
...
|mcsexec.log|Verificação de estado de funcionamento de registos, espaço de nomes, criação de sessões e certificado verifique ações.|Servidor do sistema de sites|  
...
|OfflineServicingMgr.log|Regista os detalhes de agendas de manutenção offline e a atualização de ações de aplicação ficheiros de formato WIM (Windows Imaging) do sistema operativo.|Servidor do sistema de sites|  
...
|smspxe.log|Regista detalhes sobre as respostas aos clientes que utilizem o arranque PXE e detalhes sobre a expansão de imagens de arranque e ficheiros de arranque.|Servidor do sistema de sites|  
...
|Nome do registo|Descrição|Computador com o ficheiro de registo|  
|--------------|-----------------|----------------------------|  
|pwrmgmt.log|Regista detalhes sobre atividades de gestão de energia no computador cliente, incluindo monitorização e a imposição de definições pelo agente de cliente de gestão de energia.|Cliente|  
...
|Nome do registo|Descrição|Computador com o ficheiro de registo|  
|--------------|-----------------|----------------------------|  
|hman.log|Regista informações sobre alterações da configuração do site e a publicação de informações do site para serviços de domínio do Active Directory.|Servidor do site|  
|SMSProv.log|Regista o acesso do fornecedor WMI à base de dados do site.|Computador com o Fornecedor de SMS|  
...
|outgoingcontentmanager.log|A transferência de conteúdo de registos para o Microsoft Intune.|Computador com o ponto de ligação de serviço|  
...
|SdmAgent.log|Regista detalhes sobre o controlo da remediação e compatibilidade. No entanto, o ficheiro de registo de atualizações de software, updateshandler, fornece mais detalhes informativos sobre a instalação de atualizações de software que são necessárias para a compatibilidade.<br /><br /> Este ficheiro de registo é partilhado com definições de compatibilidade.|Cliente|  
...
|StateMessage.log|Regista detalhes sobre o software atualização mensagens de estado que são criadas e enviadas para o ponto de gestão.|Cliente|  
|SUPSetup.log|Regista detalhes sobre a instalação do ponto de atualização de software. Quando a instalação de ponto de atualização de software estiver concluída, é escrito **Instalação bem-sucedida** neste ficheiro de registo.|Servidor do sistema de sites|  
...
|WCM.log|Regista detalhes sobre o software atualização configurações de pontos e as ligações ao servidor WSUS para idiomas, classificações e categorias de atualização subscritas.|Servidor do site|  
|WSUSCtrl.log|Regista detalhes sobre a configuração, a conectividade de base de dados e o estado de funcionamento do servidor WSUS do site.|Servidor do sistema de sites|  
|wsyncmgr.log|Regista detalhes sobre o software atualizam o processo de sincronização.|Servidor do site|  
...
> [!NOTE]  
>  Quando a reativação por LAN for complementada através da utilização de proxy de reativação, esta atividade é registada no cliente. Por exemplo, consulte ccmexec. log e sleepagent _ <*domínio* \> @SYSTEM_0.log no [operações de cliente](#BKMK_ClientOpLogs) secção deste tópico.  
...
|SdmAgent.log|Regista detalhes sobre o controlo da remediação e compatibilidade. No entanto, o ficheiro de registo de atualizações de software, Updateshandler, fornece mais detalhes informativos sobre a instalação de atualizações de software que são necessárias para a compatibilidade.<br /><br /> Este ficheiro de registo é partilhado com definições de compatibilidade.|Cliente|  
|ServiceWindowManager.log|Regista detalhes sobre a avaliação de janelas de manutenção.|Cliente|  
|Setupact|Ficheiro de registo primário para a maior parte dos erros que ocorrem durante o processo de instalação do Windows. O ficheiro de registo está localizado em % windir %\$Windows.~BT\sources\panther pasta.|Cliente|
|SmsWusHandler.log|Regista detalhes sobre o processo de análise da Ferramenta de Inventário das Atualizações da Microsoft.|Cliente|  
...
|Nome do registo|Descrição|Computador com o ficheiro de registo|  
|--------------|-----------------|----------------------------|  
|Change.log|Regista detalhes sobre informações de base de dados de servidor WSUS que foram alteradas.|Servidor WSUS|  
