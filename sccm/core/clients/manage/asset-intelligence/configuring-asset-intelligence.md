...
Asset Intelligence faz o inventário e gere a utilização de licenças de software.   

## <a name="steps-to-configure-asset-intelligence"></a>Passos para configurar o Asset Intelligence  
   

- **Passo 1**: para recolher os dados de inventário necessários para relatórios do Asset Intelligence, tem de ativar o agente de cliente de inventário de hardware, conforme descrito em [como expandir o inventário de hardware no System Center Configuration Manager](../../../../core/clients/manage/inventory/extend-hardware-inventory.md).
...
###  <a name="BKMK_EnableAssetIntelligence"></a> Enable Asset Intelligence hardware inventory reporting classes  
 Para ativar o Asset Intelligence em sites do Configuration Manager, tem de ativar uma ou mais inventário do Asset Intelligence hardware classes de relatório. Pode ativar as classes na home page do **Asset Intelligence** ou, na área de trabalho **Administração** , no nó **Definições do Cliente** , nas propriedades de definições de cliente. Utilize um dos seguintes procedimentos.  
...
1.  Na consola do Configuration Manager, escolha **ativos e compatibilidade** > **Asset Intelligence**.  

3.  Sobre o **home page** separador o **Asset Intelligence** de grupo, selecione **editar Classes de inventário**.   

4.  Para ativar relatórios do Asset Intelligence, selecione **ativar todas as do Asset Intelligence, classes de relatório** ou **ativar apenas do Asset Intelligence selecionadas classes de relatório**e selecione, pelo menos, um relatório classe a partir das classes apresentadas.  
...
1.  Na consola do Configuration Manager, escolha **Administration** >  **definições de cliente** > **definições predefinidas de agente de cliente**. Se tiver criado definições personalizadas de cliente, pode selecionar os em vez disso.  

3.  Sobre o **home page** separador > **propriedades** de grupo, escolha **propriedades**.   

4.  Escolher **inventário de Hardware** > **definir Classes**. .  

5.  Escolher **filtrar por categoria** > **Classes de relatório do Asset Intelligence**. A lista de classes é atualizada apenas com as classes de relatório de inventário de hardware do Asset Intelligence.  
...
3.  Adicione a função de sistema de sites do ponto de sincronização do Asset Intelligence a um servidor de sistema de sites novo ou existente:  
    ...
    -  Para uma **novo servidor de sistema de sites**: Na **home page** separador a **Create** de grupo, escolha **criar servidor do sistema de sites** para iniciar o assistente.   

        > [!NOTE]  
        >  Por predefinição, quando uma função de sistema de sites, o Configuration Manager instala os ficheiros de instalação são instalados na primeira disponível formatados em NTFS unidade de disco rígido que tem mais espaço livre em disco rígido espaço. Para impedir que o Configuration Manager instalar em unidades específicas, crie um ficheiro vazio com o nome no_sms_on_drive e copie-o para a pasta raiz da unidade antes de instalar o servidor de sistema de sites.  

    -  Para uma **servidor de sistema de sites existente**: Escolha o servidor no qual pretende instalar a função de sistema de sites do ponto de sincronização do Asset Intelligence. Ao escolher um servidor, uma lista das funções do sistema de sites que já estejam instaladas no servidor são apresentados no painel de detalhes.  

         Sobre o **home page** separador a **servidor** de grupo, selecione **Adicionar função de sistema de sites** para iniciar o assistente.  

4.  Concluir o **gerais** página. Quando adiciona o ponto de sincronização do Asset Intelligence a um servidor do sistema de sites existente, verifique os valores que foram anteriormente configurados.  

5.  Sobre o **seleção da função do sistema** página, selecione **ponto de sincronização do Asset Intelligence** na lista de funções disponíveis.  

6.  Sobre o **definições de ligação do ponto de sincronização do Asset Intelligence** página, selecione **próxima**.  
...
8.  Sobre o **definições do servidor Proxy** , especifique se o ponto de sincronização do Asset Intelligence irá utilizar um servidor proxy quando ligar ao System Center Online para sincronizar o catálogo e se deve utilizar as credenciais para ligar a o servidor de proxy.  
...
9. Na página **Agendamento da Sincronização** , especifique se pretende sincronizar catálogo do Asset Intelligence com base num agendamento. Quando ativar o agendamento da sincronização, especifique um agendamento de sincronização simples ou personalizado. Durante a sincronização agendada, o ponto de sincronização do Asset Intelligence liga ao System Center Online para obter o catálogo do Asset Intelligence mais recente. Pode sincronizar manualmente o catálogo do Asset Intelligence a partir do nó Asset Intelligence na consola do Configuration Manager. Para obter os passos sincronizar manualmente o catálogo do Asset Intelligence, consulte a [para sincronizar manualmente o catálogo do Asset Intelligence](../../../../core/clients/manage/asset-intelligence/operations-for-asset-intelligence.md#BKMK_ManuallySynchronizeCatalog) secção a [operações do Asset Intelligence no System Center Configuration Manager](../../../../core/clients/manage/asset-intelligence/operations-for-asset-intelligence.md).  
...
1.  Num computador cliente do Configuration Manager, escolha **começar** > **ferramentas administrativas** > **política de segurança Local**.  

2.  Na **política de segurança Local** caixa de diálogo em **configurações de segurança**, expanda **políticas locais**e, em seguida, selecione **política de auditoria**.  
...
1.  Num computador de controlador de domínio, escolha **começar**, aponte para **ferramentas administrativas**e, em seguida, escolha **política de segurança do domínio**.  
...
1.  Na **ativos e compatibilidade** área de trabalho, escolha **Asset Intelligence**.  

2.  Sobre o **home page** separador a **Asset Intelligence** de grupo, selecione **importar licenças de Software**.   
...
5.  Introduza o caminho UNC para o ficheiro de declaração de licença ou escolha **procurar** para selecionar uma rede de partilha de ficheiros e pastas.  
...
O Asset Intelligence faz corresponder os produtos que especificar na declaração de licença geral, utilizando o nome do produto e a versão do produto, mas não o nome do fabricante. Tem de utilizar um nome de produto na declaração de licença geral que seja uma correspondência exata com o nome de produto armazenado na base de dados do site. Do Asset Intelligence utiliza a **Quantidadeefetiva** número indicado na declaração de licença geral e compara o número com o número de produtos instalados encontrado no inventário do Configuration Manager.  

> [!TIP]  
>  Para obter uma lista completa dos nomes de produto armazenados na base de dados do site do Configuration Manager, pode executar a consulta seguinte na base de dados do site: SELECT ProductName0 FROM v_GS_INSTALLED_SOFTWARE.  
...
7.  Execute o Asset Intelligence **licença 15A - relatório de reconciliação de Software de terceiros** para verificar se as informações de licenciamento foram importadas com êxito para o catálogo do Asset Intelligence.  

> [!NOTE]  
>  Para obter um exemplo de um ficheiro de licença de software geral que pode utilizar para fins de teste, consulte [ficheiro de importação de licença geral do exemplo Asset Intelligence no System Center Configuration Manager](../../../../core/clients/manage/asset-intelligence/example-asset-intelligence-general-license-import.md).  
...
-   **Verificar o título da aplicação nas informações de inventário**: Verifica se o título de software comunicado no inventário de software é reconciliado com o título de software no catálogo do Asset Intelligence. Por predefinição, esta tarefa é ativada e agendada para ser executada no Sábado após 12 horas e antes das 5:00 Esta tarefa de manutenção só está disponível no site de nível superior na hierarquia do Configuration Manager.  

-   **Resumir dados de Software instalado**: Fornece as informações que são apresentadas no **ativos e compatibilidade** área de trabalho, na **Software inventariado** nó, no **do Asset Intelligence** nó. Quando a tarefa é executada, o Configuration Manager recolhe uma contagem de todos os títulos de software inventariados no site primário. Por predefinição, esta tarefa é ativada e agendada para ser executada todos os dias após 12 horas e antes das 5:00 Esta tarefa de manutenção está disponível apenas em sites primários.  
...
4.  Sobre o **home page** separador a **definições** de grupo, selecione **manutenção do Site**. Selecione uma tarefa e escolha **editar** para modificar as definições. 

    Recomendamos que defina o período de tempo para horas de ponta do site. O período de tempo corresponde ao intervalo de tempo em que a tarefa pode ser executada. É definido pelos campos **Iniciar após** e **Última hora de início** especificados na caixa de diálogo **Propriedades da Tarefa** .  
...
7.  Escolher **OK** para guardar as definições. A tarefa é agora executada de acordo com o respetivo agendamento.  

    > [!NOTE]  
    >  Se uma tarefa falhar executar na primeira tentativa, o Configuration Manager tenta executar novamente a tarefa até que a tarefa é executada com êxito ou até que o período de tempo em que pode executar a tarefa tenha passado.  
