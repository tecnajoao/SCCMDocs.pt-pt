---
title: Configurar o Asset Intelligence | Documentos do Microsoft
...
O Asset Intelligence fará um inventário dos e gere a utilização de licenças de software.   
...
- **Passo 4**: [Ativar a auditoria dos eventos de início de sessão bem sucedido](#BKMK_EnableSuccessLogonEvents)  
...
###  <a name="BKMK_EnableAssetIntelligence"></a> Enable Asset Intelligence hardware inventory reporting classes  
 Para ativar o Asset Intelligence no sites do Configuration Manager, tem de ativar uma ou mais Asset Intelligence inventário de hardware classes de relatório. Pode ativar as classes na home page do **Asset Intelligence** ou, na área de trabalho **Administração** , no nó **Definições do Cliente** , nas propriedades de definições de cliente. Utilize um dos seguintes procedimentos.  
...
3.  No **base** separador o **Asset Intelligence** grupo, selecione **editar Classes de inventário**.   

4.  Para ativar a denúncia de Asset Intelligence, selecione **ativar todos os relatório do Asset Intelligence classes** ou **ativar apenas o Asset Intelligence selecionadas classes de relatório**e selecione, pelo menos, uma classe de relatório das classes apresentadas.  
...
1.  Na consola do Configuration Manager, escolha **administração** >  **definições de cliente** > **agente predefinições de cliente**. Se tiver criado personalizadas de cliente definições, pode selecionar os em vez disso.  

3.  No **base** separador > **propriedades** grupo, selecione **propriedades**.   
...
3.  Adicione a função de sistema de sites do ponto de sincronização do Asset Intelligence para um servidor do sistema de sites novo ou existente:  

    -  Para um **novo servidor de sistema de sites**: No **base** separador o **criar** grupo, selecione **criar servidor do sistema de sites** para iniciar o assistente.   

        > [!NOTE]  
        >  Por predefinição, quando o Configuration Manager instala uma função de sistema de sites, os ficheiros de instalação são instalados na primeira disponível formatados como NTFS unidade de disco rígido que tenha mais disco rígido livre espaço. Para impedir que o Configuration Manager instalar em unidades específicas, crie um ficheiro vazio designado no_sms_on_drive e copie-o para a pasta de raiz da unidade antes de instalar o servidor de sistema de sites.  

    -  Para um **servidor de sistema de sites existente**: Selecione o servidor no qual pretende instalar a função de sistema de sites do ponto de sincronização do Asset Intelligence. Quando escolher um servidor, uma lista das funções de sistema de sites que já estão instaladas no servidor são apresentados no painel de detalhes.  

         No **base** separador o **servidor** grupo, selecione **Adicionar função de sistema de sites** para iniciar o assistente.  
...
6.  No **definições de ligação do ponto de sincronização do Asset Intelligence** página, selecione **seguinte**.  
...
8.  No **definições do servidor Proxy** página, especifique se o ponto de sincronização do Asset Intelligence irá utilizar um servidor proxy ao ligar-se ao System Center Online para sincronizar o catálogo e se pretende utilizar credenciais para ligar ao servidor proxy.  
...
1.  Num computador do controlador de domínio, selecione **iniciar**, aponte para **ferramentas administrativas**e, em seguida, escolha **política de segurança do domínio**.  
...
1.  No **ativos e compatibilidade** área de trabalho, selecione **Asset Intelligence**.  

2.  No **base** separador o **Asset Intelligence** grupo, selecione **importar licenças de Software**.   
...
5.  Introduza o caminho UNC para o ficheiro da declaração de licença ou escolha **procurar** selecionar uma rede partilhada ficheiros e pastas.  
...
> [!NOTE]  
>  Embora apenas os campos **Nome**, **Fabricante**, **Versão**, e **QuantidadeEfetiva** tenham de conter dados, todos os campos têm de ser introduzidos na primeira linha do ficheiro de importação de licença. Todos os campos de data devem ser apresentados no seguinte formato: Mês/dia/ano, por exemplo, 04/08/2008.  

O Asset Intelligence faz corresponder os produtos que especificar na declaração de licença geral, utilizando o nome do produto e a versão do produto, mas não o nome do fabricante. Tem de utilizar um nome de produto na declaração de licença geral que seja uma correspondência exata com o nome de produto armazenado na base de dados do site. O Asset Intelligence demora a **EffectiveQuantity** número fornecido na declaração de licença geral e compara o número com o número de produtos instalados foi encontrado no inventário do Configuration Manager.  

> [!TIP]  
>  Para obter uma lista completa dos nomes de produtos armazenados na base de dados de site do Configuration Manager, pode executar a consulta seguinte na base de dados do site: SELECIONE ProductName0 de v_GS_INSTALLED_SOFTWARE.  
...
7.  Executar o Asset Intelligence **licença 15A - relatório de reconciliação de Software de terceiros** para verificar que as informações de licenciamento foi importadas com êxito para o catálogo de Asset Intelligence.  

> [!NOTE]  
>  Para obter um exemplo de um ficheiro de licença de software gerais que podem ser utilizados para fins de teste, consulte o artigo [ficheiro de importação de licenças gerais de exemplo do Asset Intelligence no System Center Configuration Manager](../../../../core/clients/manage/asset-intelligence/example-asset-intelligence-general-license-import.md).  
...
-   **Verificar o título da aplicação nas informações de inventário**: Verifica se o título de software comunicados no inventário de software incluído no título de software no catálogo do Asset Intelligence. Por predefinição, esta tarefa é ativada e agendada para ser executada no Sábado após 12:00 da manhã. e antes de 5:00 da manhã. Esta tarefa de manutenção só está disponível no site de nível superior na sua hierarquia do Configuration Manager.  

-   **Resumir os dados do Software instalado**: Fornece as informações que são apresentadas no **ativos e compatibilidade** área de trabalho, no **Software inventariado, localizado** nó, no **Asset Intelligence** nó. Quando a tarefa é executada, o Configuration Manager reúne uma contagem de todos os títulos de software inventariado no site primário. Por predefinição, esta tarefa é ativada e agendada para ser executada diariamente após 12:00 da manhã. e antes de 5:00 da manhã. Esta tarefa de manutenção está disponível apenas em sites primários.  
...
4.  No **base** separador o **definições** grupo, selecione **manutenção do Site**. Selecione uma tarefa e escolha **editar** para modificar as definições. 
...
7.  Escolher **OK** para guardar as definições. A tarefa é agora executada de acordo com o respetivo agendamento.  
    ...
    > [!NOTE]  
    >  Se uma tarefa não for executado durante a primeira tentativa, Configuration Manager tenta executar novamente a tarefa, até a tarefa a ser executado com êxito ou até que o período de tempo em que pode executar a tarefa tiver passado.  
